				...
				(function () {
					...
					function printWarning(level, format, args) {
						{
							var ReactDebugCurrentFrame2 =
								ReactSharedInternals.ReactDebugCurrentFrame;
							var stack =
								ReactDebugCurrentFrame2.getStackAddendum();
							if (stack !== '') {
								format += '%s';
						...
						if (typeof type === 'object') {
							switch (type.$$typeof) {
								case REACT_CONTEXT_TYPE:
									var context = type;
									return (
										getContextName(context) + '.Consumer'
									);
					...
					function createElement(type, config, children) {
						...
						if (config != null) {
							...
							self =
								config.__self === void 0 ? null : config.__self;
							source =
								config.__source === void 0
									? null
									: config.__source;
					...
					function mapIntoArray(
						...
						if (Array.isArray(children)) {
							...
							if (typeof iteratorFn === 'function') {
								...
								var iterator =
									iteratorFn.call(iterableChildren);
								var step;
								var ii = 0;
					...
					function forwardRef(render2) {
						{
							...
							if (render2 != null) {
								if (
									render2.defaultProps != null ||
									render2.propTypes != null
								) {
									error(
			...
			if (true) {
				(function () {
					...
					if (
						...
						requestPaint = exports.unstable_forceFrameRate =
							function () {};
					} else {
				...
				(function () {
							...
							if (node.namespaceURI === Namespaces.svg) {
								if (!('innerHTML' in node)) {
									...
									reusableSVGContainer.innerHTML =
										...
										'</svg>';
									var svgNode =
										reusableSVGContainer.firstChild;
					...
					function validateShorthandPropertyCollisionInDev(
					...
					) {
						{
							...
							var expandedUpdates =
								expandShorthandMap(styleUpdates);
					...
					function restoreStateOfTarget(target) {
						...
						if (stateNode) {
							var _props =
								getFiberCurrentPropsFromNode(stateNode);
							restoreImpl(
							...
							if (
								...
								if (!instance._warnedAboutRefsInRender) {
									error(
										...
										getComponentName(ownerFiber.type) ||
											'A component'
									);
								}
					...
					function findCurrentHostFiber(parent) {
						var currentParent =
							findCurrentFiberUsingSlowPath(parent);
						if (!currentParent) {
							return null;
						}
					...
					function accumulateOrCreateContinuousQueuedReplayableEvent(
						...
						var targetContainers =
							existingQueuedEvent.targetContainers;
						if (
					...
					function replayUnblockedEvents() {
						...
						while (queuedDiscreteEvents.length > 0) {
							...
							var targetContainers =
								nextDiscreteEvent.targetContainers;
							while (targetContainers.length > 0) {
								var targetContainer = targetContainers[0];
								var nextBlockedOn = attemptToDispatchEvent(
					...
					function retryIfBlockedOn(unblocked) {
						...
						for (
							...
							var queuedTarget =
								queuedExplicitHydrationTargets[_i];
					...
					function getNextLanes(root2, wipLanes) {
						...
						if (expiredLanes !== NoLanes) {
							...
							if (nonIdlePendingLanes !== NoLanes) {
								...
								if (unblockedLanes !== NoLanes) {
									nextLanes =
										getHighestPriorityLanes(unblockedLanes);
									nextLanePriority =
										return_highestLanePriority;
								} else {
						...
						nextLanes =
							pendingLanes &
							getEqualOrHigherPriorityLanes(nextLanes);
						if (
							wipLanes !== NoLanes &&
							wipLanes !== nextLanes &&
						...
						switch (lanePriority) {
							...
							case DefaultLanePriority: {
								var _lane3 = pickArbitraryLane(
									DefaultLanes & ~wipLanes
								);
								if (_lane3 === NoLane) {
									_lane3 = pickArbitraryLane(
										TransitionLanes & ~wipLanes
									);
									if (_lane3 === NoLane) {
										_lane3 =
					...
					function markDiscreteUpdatesExpired(root2) {
						root2.expiredLanes |=
							InputDiscreteLanes & root2.pendingLanes;
					}
					function hasDiscreteLanes(lanes) {
						return (lanes & InputDiscreteLanes) !== NoLanes;
					}
					function markRootMutableRead(root2, updateLane) {
						root2.mutableReadLanes |=
							updateLane & root2.pendingLanes;
					}
					...
					function isEventSupported(eventNameSuffix) {
						...
						if (!isSupported) {
							...
							isSupported =
								typeof element[eventName] === 'function';
						}
						return isSupported;
					}
					...
					function processDispatchQueueItemsInOrder(
						...
						if (inCapturePhase) {
							...
							for (
								var _i = 0;
								_i < dispatchListeners.length;
								_i++
							) {
								var _dispatchListeners$_i =
										dispatchListeners[_i],
					...
					var updatedAncestorInfo = function () {};
					{
						...
						var isTagValidWithParent = function (tag, parentTag) {
							...
							switch (tag) {
								...
								case 'rt':
									return (
										impliedEndTags.indexOf(parentTag) === -1
									);
								case 'body':
								case 'caption':
								case 'col':
							...
							case DOCUMENT_FRAGMENT_NODE: {
								type =
									nodeType === DOCUMENT_NODE
										...
										: '#fragment';
								var root2 =
									rootContainerInstance.documentElement;
								namespace = root2
									? root2.namespaceURI
					...
					function onCommitUnmount(fiber) {
						if (
							injectedHook &&
							typeof injectedHook.onCommitFiberUnmount ===
								'function'
						) {
							try {
					...
					};
					{
						...
						ReactStrictModeWarnings.flushPendingUnsafeLifecycleWarnings =
							function () {
								...
								if (
									pendingComponentWillUpdateWarnings.length >
									0
								) {
								...
								if (
									UNSAFE_componentWillMountUniqueNames.size >
									0
								) {
								...
								if (
									UNSAFE_componentWillUpdateUniqueNames.size >
									0
								) {
								...
								if (
									componentWillReceivePropsUniqueNames.size >
									0
								) {
					...
					function calculateChangedBits(context, newValue, oldValue) {
						if (objectIs(oldValue, newValue)) {
							...
							var changedBits =
								typeof context._calculateChangedBits ===
								'function'
									? context._calculateChangedBits(
											oldValue,
											newValue
					...
					function scheduleWorkOnParentPath(parent, renderLanes2) {
						...
						while (node !== null) {
							...
							if (
								!isSubsetOfLanes(node.childLanes, renderLanes2)
							) {
					...
					function processUpdateQueue(
						...
						if (firstBaseUpdate !== null) {
							...
							do {
								...
								if (
									!isSubsetOfLanes(renderLanes2, updateLane)
								) {
									var clone = {
					...
					function checkClassInstance(
						...
						var instance = workInProgress2.stateNode;
						{
							...
							if (
								typeof instance.componentShouldUpdate ===
								'function'
							) {
								error(
							...
							if (
								typeof instance.componentDidUnmount ===
								'function'
							) {
								error(
							...
							if (
								typeof instance.getSnapshotBeforeUpdate ===
									'function' &&
								typeof instance.componentDidUpdate !==
									'function' &&
						...
						if (oldState !== instance.state) {
							{
								error(
									...
									getComponentName(workInProgress2.type) ||
										'Component'
								);
							}
									...
									switch (child.tag) {
										...
										default: {
											if (
												...
												{
													_existing3._debugSource =
														element._source;
													_existing3._debugOwner =
														element._owner;
												}
												return _existing3;
											}
					...
					function tryToClaimNextHydratableInstance(fiber) {
						...
						hydrationParentFiber = fiber;
						nextHydratableInstance =
							getFirstHydratableChild(nextInstance);
					}
					...
					function resetWorkInProgressVersions() {
						for (var i = 0; i < workInProgressSources.length; i++) {
							...
							{
								mutableSource._workInProgressVersionPrimary =
									null;
							}
						}
					...
					function warnAboutMultipleRenderersDEV(mutableSource) {
						{
							{
								if (
									mutableSource._currentPrimaryRenderer ==
									null
								) {
									mutableSource._currentPrimaryRenderer =
										rendererSigil$1;
					...
					function readFromUnsubcribedMutableSource(
						...
						var currentRenderVersion =
							getWorkInProgressVersion(source);
						if (currentRenderVersion !== null) {
							isSafeToReadFromSource =
								currentRenderVersion === version;
						} else {
						...
						dispatcher.useEffect(
							function () {
								var handleChange = function () {
									...
									try {
										latestSetSnapshot(
											latestGetSnapshot(source._source)
										);
										var lane = requestUpdateLane(fiber);
										markRootMutableRead(root2, lane);
									} catch (error2) {
					...
					function mountOpaqueIdentifier() {
						...
						if (getIsHydrating()) {
							...
							if (
								(currentlyRenderingFiber$1.mode &
									BlockingMode) ===
								NoMode
							) {
					...
					function updateSimpleMemoComponent(
						...
						if (current2 !== null) {
							...
							if (
								...
								if (
									!includesSomeLane(renderLanes2, updateLanes)
								) {
						...
						) {
							{
								...
								if (mutableSourceEagerHydrationData != null) {
									for (
										var i = 0;
										i <
										mutableSourceEagerHydrationData.length;
										i += 2
									) {
										var mutableSource =
					...
					function validateFunctionComponentInDev(
					...
					) {
						{
							...
							if (workInProgress2.ref !== null) {
								...
								var warningKey =
									ownerName || workInProgress2._debugID || '';
						...
						if (
							...
							if (
								...
								if (
									...
									nextProps.unstable_avoidThisFallback !==
										true
								) {
					...
					function mountSuspenseFallbackChildren(
						...
						if (
							...
							primaryChildFragment.pendingProps =
								primaryChildProps;
					...
					function updateSuspenseFallbackChildren(
						...
						if (
							...
							if (progressedLastEffect !== null) {
								workInProgress2.firstEffect =
									primaryChildFragment.firstEffect;
								workInProgress2.lastEffect =
					...
					function updateContextProvider(
						...
						var newValue = newProps.value;
						{
							...
							var providerPropTypes =
								workInProgress2.type.propTypes;
					...
					function remountFiber(
					...
					) {
						{
							...
							newWorkInProgress.sibling =
								oldWorkInProgress.sibling;
							...
							if (oldWorkInProgress === returnFiber.child) {
								...
								while (
									prevSibling.sibling !== oldWorkInProgress
								) {
					...
					function logCapturedError(boundary, errorInfo) {
						try {
							...
							if (true) {
								...
								var errorBoundaryName = getComponentName(
									boundary.type
								);
						...
						if (wakeables !== null) {
							...
							wakeables.forEach(function (wakeable) {
								...
								if (!retryCache.has(wakeable)) {
									{
										if (
											wakeable.__reactDoNotTraceInteractions !==
											true
										) {
											retry =
												tracing.unstable_wrap(retry);
										}
									}
					...
					function isSuspenseBoundaryBeingHidden(
						...
						if (current2 !== null) {
							...
							if (
								oldState === null ||
								oldState.dehydrated !== null
							) {
								var newState = finishedWork.memoizedState;
								return (
									newState !== null &&
									newState.dehydrated === null
								);
							}
						}
						return false;
					...
					if (typeof Symbol === 'function' && Symbol.for) {
						...
						HAS_PSEUDO_CLASS_TYPE = symbolFor$1(
					...
					function ensureRootIsScheduled(root2, currentTime) {
						...
						if (newCallbackPriority === SyncLanePriority) {
							...
							newCallbackPriority === SyncBatchedLanePriority
						) {
					...
					function pushDispatcher() {
						...
						ReactCurrentDispatcher$2.current =
							ContextOnlyDispatcher;
					...
					function completeUnitOfWork(unitOfWork) {
						...
						do {
							...
							if (
								(completedWork.flags & Incomplete) ===
								NoFlags
							) {
					...
					function commitRootImpl(root2, renderPriorityLevel) {
						...
						if (rootsWithPendingDiscreteUpdates !== null) {
							if (
								...
								rootsWithPendingDiscreteUpdates.has(root2)
							) {
						...
						if (finishedWork.flags > PerformedWork) {
							if (finishedWork.lastEffect !== null) {
								finishedWork.lastEffect.nextEffect =
									finishedWork;
					...
					function captureCommitPhaseErrorOnRoot(
						...
						var eventTime = requestEventTime();
						var root2 = markUpdateLaneFromFiberToRoot(
							rootFiber,
							SyncLane
						);
						if (root2 !== null) {
...
For more info, visit https://reactjs.org/link/mock-scheduler`);
					...
					function scheduleInteractions(root2, lane, interactions) {
						if (interactions.size > 0) {
							var pendingInteractionMap =
								root2.pendingInteractionMap;
							var pendingInteractions =
								pendingInteractionMap.get(lane);
					...
					function startWorkOnPendingInteractions(root2, lanes) {
						...
						root2.pendingInteractionMap.forEach(function (
							...
							if (includesSomeLane(lanes, scheduledLane)) {
								scheduledInteractions.forEach(function (
									interaction
								) {
					...
					function finishPendingInteractions(root2, committedLanes) {
						...
						try {
							...
							if (
								subscriber !== null &&
								root2.memoizedInteractions.size > 0
							) {
					...
					function findHostInstanceWithNoPortals(fiber) {
						var hostFiber =
							findCurrentHostFiberWithNoPortals(fiber);
						if (hostFiber === null) {
							return null;
						}
					...
					function createRootImpl(container, tag, options2) {
						var hydrate2 =
							options2 != null && options2.hydrate === true;
						var hydrationCallbacks =
							(options2 != null && options2.hydrationOptions) ||
							null;
						var mutableSources =
							(options2 != null &&
